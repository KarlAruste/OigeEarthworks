...
def estimate_length_area_volume_from_tin(
    ...
    """
    Heuristic TIN -> planned length, mean cross-section area, volume.

    Steps:
    - PCA on XY -> main direction 's'
    - For many slices along s, take points near slice center
    - In each slice estimate "excavation area" vs top plane (z_top percentile)
      area ≈ ∫ max(0, z_top - z(t)) dt
    """
    try:
    ...
    main_dir = eigvecs[:, np.argmax(eigvals)]  # unit-ish vector
    ...
    for c in centers:
        ...
        # reference top
        z_top = float(np.percentile(zz, top_percentile))
