...
def set_worker_active(worker_id: int, active: bool):
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("UPDATE workers SET active=%s WHERE id=%s;", (bool(active), worker_id))
        conn.commit()
...
def _missing_deps_count(cur, task_id: int) -> int:
    ...
    return int(cur.fetchone()["missing"] or 0)
